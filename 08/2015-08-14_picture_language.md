...
现在感觉应该到了第三境界，现写出来希望与大家分享这个奇妙的图形语言。
  ...
  (let ((quarter (corner-split painter n)))
    (let ((half (beside (flip-horiz quarter) quarter)))
      (below (flip-vert half) 
             half))))
```
我们如果用Racket中提供的该图形语言的实现（配置可参考[这里]((/exercises/02/2.44_2.45.md))），按照下面的调用方式
```
...
看到这里，算是把这个图形语言彻底理解了，可能你才发现，我们至今没有提及painter的实现，但是这毫无影响我们对其进行变换，这就是分层设计的威力，只需要关心其暴露出来的借口。 
